...
accel_data = accel_data %>% janitor::clean_names() %>% 
  ...
  select(week, day_id, day, weekend, activity_1:activity_1440)
...
1-5, a day ID variable ranging from 1-35, a day variable and a weekend
variable, and 1440 variables for each minute of the day that measures
activity count. There are 35 observations.
...
accel_long %>% ggplot(aes(activity_minute, activity, group = day, color = day)) +
  geom_line(alpha = 0.5)
```

<img src="p8105_hw3_lef2147_files/figure-gfm/unnamed-chunk-16-1.png" width="90%" />
